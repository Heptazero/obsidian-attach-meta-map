...
- **folder**：只配置一个**资源集合文件夹**。松散资源和整理后的条目都属于这个集合；插件在同一根目录下新建和笔记同名的子文件夹，把资源搬进去，笔记也建在里面。资源保留自己的文件名，只改变位置。

folder 布局不再有“附件文件夹 / 笔记文件夹”两个字段，也没有“镜像子文件夹”。只有集合根目录里的直接文件会被整理；进入任意一级子文件夹后，就永久退出自动移动、自动建笔记和批量补齐。文件夹可以随意改名，不依赖同名判断，因此不会再次嵌套。插件不监听 `delete`；已有配对只由笔记中的 `source` 决定。
